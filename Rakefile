...
namespace :proto do
  ...
  task :generate do
    Dir.mkdir(PROTOBUF_PATH) unless Dir.exist?(PROTOBUF_PATH)
    Dir.mkdir(RBS_SIG_PATH) unless Dir.exist?(RBS_SIG_PATH)
    Dir.mkdir(GRPC_PATH) unless Dir.exist?(GRPC_PATH)
