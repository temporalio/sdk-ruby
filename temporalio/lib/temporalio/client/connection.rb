...
module Temporalio
  class Client
    ...
    class Connection
      ...
      #
      # @!attribute initial_interval_ms
      #   @return [Integer] Initial backoff interval, default 100.
      ...
      # @!attribute max_interval_ms
      #   @return [Integer] Maximum backoff interval, default 5000.
      # @!attribute max_elapsed_time_ms
      #   @return [Integer] Maximum total time, default 10000.
      ...
      RPCRetryOptions = Struct.new(
        ...
        def initialize(*, **kwargs)
          kwargs[:initial_interval_ms] = 100 unless kwargs.key?(:initial_interval_ms)
          kwargs[:randomization_factor] = 0.2 unless kwargs.key?(:randomization_factor)
          kwargs[:multiplier] = 1.5 unless kwargs.key?(:multiplier)
          kwargs[:max_interval_ms] = 5000 unless kwargs.key?(:max_interval_ms)
          kwargs[:max_elapsed_time_ms] = 10_000 unless kwargs.key?(:max_elapsed_time_ms)
      ...
      #   @return [Integer] Interval to send HTTP2 keep alive pings, default 30000.
      # @!attribute timeout_ms
      #   @return [Integer] Timeout that the keep alive must be responded to within or the connection will be closed,
      #     default 15000.
      KeepAliveOptions = Struct.new(
        ...
        def initialize(*, **kwargs)
          kwargs[:interval_ms] = 30_000 unless kwargs.key?(:interval_ms)
          kwargs[:timeout_ms] = 15_000 unless kwargs.key?(:timeout_ms)
          super
        end
      end
      ...
      def new_core_client
        options = Internal::Bridge::Client::Options.new(
          ...
          rpc_retry: Internal::Bridge::Client::RPCRetryOptions.new(
            initial_interval_ms: @options.rpc_retry.initial_interval_ms,
            randomization_factor: @options.rpc_retry.randomization_factor,
            multiplier: @options.rpc_retry.multiplier,
            max_interval_ms: @options.rpc_retry.max_interval_ms,
            max_elapsed_time_ms: @options.rpc_retry.max_elapsed_time_ms,
        ...
        if @options.keep_alive
          options.keep_alive = Internal::Bridge::Client::KeepAliveOptions.new(
            interval_ms: @options.keep_alive.interval_ms,
            timeout_ms: @options.keep_alive.timeout_ms
          )
        end
