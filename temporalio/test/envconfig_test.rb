...
class EnvConfigTest < Test
  # Test fixtures for comprehensive coverage following the ideal 35-test suite
  # designed for consistency across Python, TypeScript, .NET, and Ruby SDKs
  ...
  def test_load_profile_from_file_default
    with_temp_config_file(TOML_CONFIG_BASE) do |config_file|
      profile = Temporalio::EnvConfig::ClientConfigProfile.load(config_source: config_file)
  ...
  def test_load_profile_from_file_custom
    with_temp_config_file(TOML_CONFIG_BASE) do |config_file|
      profile = Temporalio::EnvConfig::ClientConfigProfile.load(config_source: config_file, profile: 'custom')
  ...
  def test_load_profile_env_overrides
    with_temp_config_file(TOML_CONFIG_BASE) do |config_file|
      ...
      profile = Temporalio::EnvConfig::ClientConfigProfile.load(
        config_source: config_file, profile: 'custom', override_env_vars: env
      )
      assert_equal 'env-address', profile.address
      assert_equal 'env-namespace', profile.namespace
  ...
  def test_load_profile_disable_env
    with_temp_config_file(TOML_CONFIG_BASE) do |config_file|
      ...
      profile = Temporalio::EnvConfig::ClientConfigProfile.load(
        config_source: config_file, override_env_vars: env, disable_env: true
      )
      assert_equal 'default-address', profile.address
  ...
  def test_load_profiles_from_file_all
    with_temp_config_file(TOML_CONFIG_BASE) do |config_file|
      client_config = Temporalio::EnvConfig::ClientConfig.load(config_source: config_file)
  ...
  def test_load_profile_not_found
    with_temp_config_file(TOML_CONFIG_BASE) do |config_file|
      assert_raises(Temporalio::Internal::Bridge::Error) do
        Temporalio::EnvConfig::ClientConfigProfile.load(config_source: config_file, profile: 'nonexistent')
      end
    end
  end
  ...
  def test_load_profiles_strict_mode_fail
    with_temp_config_file(TOML_CONFIG_STRICT_FAIL) do |config_file|
      assert_raises(Temporalio::Internal::Bridge::Error) do
        Temporalio::EnvConfig::ClientConfig.load(config_source: config_file, config_file_strict: true)
      end
    end
  end
  ...
  def test_load_profile_strict_mode_fail
    with_temp_config_file(TOML_CONFIG_STRICT_FAIL) do |config_file|
      assert_raises(Temporalio::Internal::Bridge::Error) do
        Temporalio::EnvConfig::ClientConfigProfile.load(config_source: config_file, config_file_strict: true)
      end
    end
  end
  ...
  def test_e2e_production_tls_api_key_client_connection
    ...
    assert_not_nil client.connection.options.tls # TLS should be enabled with API key
  ...
  def test_e2e_multi_profile_different_client_connections
    ...
    assert_not_nil prod_client.connection.options.tls # TLS enabled with API key
  end
