...
describe Temporal::Worker do
  ...
  let(:start) { instance_double("Temporal::Start", workflow_execution: workflow_execution, activity_id: "test-activity-id-1") }
  ...
  describe "#run" do
    it "polls and executes activity tasks in a loop" do
      ...
      thread2 = Thread.new { sleep(0.1); worker.terminate }
