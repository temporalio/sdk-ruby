...
describe Temporalio::FailureConverter::Basic do
  subject { described_class.new }
  let(:converter) { Temporalio::PayloadConverter::DEFAULT }
  let(:details) { { 'foo' => 'bar' } }
  ...
  describe '#to_failure' do
    ...
    context 'with ApplicationError' do
      ...
      it 'returns a failure' do
        ...
        expect(failure).to be_a(Temporalio::Api::Failure::V1::Failure)
    ...
    context 'with TimeoutError' do
      let(:error) do
        Temporalio::Error::TimeoutError.new(
          'Test error',
          type: Temporalio::TimeoutType::SCHEDULE_TO_START,
    ...
    context 'with CancelledError' do
      let(:error) do
        Temporalio::Error::CancelledError.new('Test error', details: [details])
      end
    ...
    context 'with TerminatedError' do
      let(:error) do
        Temporalio::Error::TerminatedError.new('Test error')
      end
    ...
    context 'with ServerError' do
      let(:error) do
        Temporalio::Error::ServerError.new('Test error', non_retryable: true)
      end
    ...
    context 'with ResetWorkflowError' do
      let(:error) do
        Temporalio::Error::ResetWorkflowError.new('Test error', last_heartbeat_details: [details])
      end
    ...
    context 'with ActivityError' do
      let(:error) do
        Temporalio::Error::ActivityError.new(
          ...
          retry_state: Temporalio::RetryState::MAXIMUM_ATTEMPTS_REACHED,
        )
      end
    ...
    context 'when encode_common_attributes is set to true' do
      ...
      it 'encodes attributes' do
        ...
        expect(failure.encoded_attributes).to be_a(Temporalio::Api::Common::V1::Payload)
  ...
  describe '#from_failure' do
    ...
    context 'with missing failure info' do
      it 'returns an error' do
        ...
        expect(error).to be_a(Temporalio::Error::Failure)
    ...
    context 'with timeout failure info' do
      let(:failure_info) do
        Temporalio::Api::Failure::V1::TimeoutFailureInfo.new(
          timeout_type: Temporalio::Api::Enums::V1::TimeoutType::TIMEOUT_TYPE_SCHEDULE_TO_START,
      ...
      it 'returns an error' do
        ...
        expect(error).to be_a(Temporalio::Error::TimeoutError)
        expect(error.message).to eq('Test failure')
        expect(error.type).to eq(Temporalio::TimeoutType::SCHEDULE_TO_START)
    ...
    context 'with canceled failure info' do
      let(:failure_info) do
        Temporalio::Api::Failure::V1::CanceledFailureInfo.new(
          details: { payloads: [payload] },
        )
      end
      ...
      it 'returns an error' do
        ...
        expect(error).to be_a(Temporalio::Error::CancelledError)
        expect(error.message).to eq('Test failure')
        expect(error.details).to eq([details])
        expect(error.raw).to eq(failure)
        expect(error.backtrace).to eq(backtrace)
    ...
    context 'with terminated failure info' do
      let(:failure_info) { Temporalio::Api::Failure::V1::TerminatedFailureInfo.new }
      ...
      it 'returns an error' do
        ...
        expect(error).to be_a(Temporalio::Error::TerminatedError)
        expect(error.message).to eq('Test failure')
        expect(error.raw).to eq(failure)
        expect(error.backtrace).to eq(backtrace)
        expect(error.cause).to eq(nil)
    ...
    context 'with server failure info' do
      let(:failure_info) { Temporalio::Api::Failure::V1::ServerFailureInfo.new(non_retryable: true) }
      ...
      it 'returns an error' do
        ...
        expect(error).to be_a(Temporalio::Error::ServerError)
        expect(error.message).to eq('Test failure')
        expect(error.non_retryable).to eq(true)
        expect(error).not_to be_retryable
        expect(error.raw).to eq(failure)
    ...
    context 'with reset workflow failure info' do
      let(:failure_info) do
        Temporalio::Api::Failure::V1::ResetWorkflowFailureInfo.new(
          last_heartbeat_details: { payloads: [payload] },
        )
      end
      ...
      it 'returns an error' do
        ...
        expect(error).to be_a(Temporalio::Error::ResetWorkflowError)
        expect(error.message).to eq('Test failure')
        expect(error.last_heartbeat_details).to eq([details])
        expect(error.raw).to eq(failure)
        expect(error.backtrace).to eq(backtrace)
    ...
    context 'with activity failure info' do
      let(:failure_info) do
        Temporalio::Api::Failure::V1::ActivityFailureInfo.new(
          scheduled_event_id: 1,
          started_event_id: 2,
          identity: 'test-identity',
          activity_type: { name: 'test-activity' },
          activity_id: 'test-activity-id',
          retry_state: Temporalio::Api::Enums::V1::RetryState::RETRY_STATE_MAXIMUM_ATTEMPTS_REACHED,
        )
      end
      ...
      it 'returns an error' do
        ...
        expect(error).to be_a(Temporalio::Error::ActivityError)
        expect(error.message).to eq('Test failure')
        ...
        expect(error.retry_state).to eq(Temporalio::RetryState::MAXIMUM_ATTEMPTS_REACHED)
        expect(error.raw).to eq(failure)
        expect(error.backtrace).to eq(backtrace)
        expect(error.cause).to eq(nil)
      end
    ...
    context 'with child workflow execution failure info' do
      ...
      it 'returns an error' do
        ...
        expect(error).to be_a(Temporalio::Error::ChildWorkflowError)
        expect(error.message).to eq('Test failure')
