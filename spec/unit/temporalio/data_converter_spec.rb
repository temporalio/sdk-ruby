...
require 'temporalio/payload_codec/base'

class TestConcatenatingPayloadCodec < Temporalio::PayloadCodec::Base
  ...
  def encode(payloads)
    data = payloads.map do |payload|
      Temporalio::Api::Common::V1::Payload.encode(payload)
    end.join(separator)
...
class TestFaultyPayloadCodec < Temporalio::PayloadCodec::Base
...
class TestEncodingSwappingPayloadCodec < Temporalio::PayloadCodec::Base
...
describe Temporalio::DataConverter do
  ...
  let(:codecs) { [] }
  let(:failure_converter) { Temporalio::FailureConverter::DEFAULT }
  let(:nil_payload) do
    Temporalio::Api::Common::V1::Payload.new(
      metadata: { 'encoding' => Temporalio::PayloadConverter::Nil::ENCODING },
    )
  end
  let(:json_payload) do
    Temporalio::Api::Common::V1::Payload.new(
  ...
  describe '#to_payload' do
    it 'calls the provided payload converter' do
      ...
      expect(result).to be_a(Temporalio::Api::Common::V1::Payload)
  ...
  describe '#to_payloads' do
    ...
    it 'converts a single value to payloads' do
      ...
      expect(result).to be_a(Temporalio::Api::Common::V1::Payloads)
  ...
  describe '#to_failure' do
    let(:error) do
      Temporalio::Error::ApplicationError.new('test error', type: 'test type', details: 'test')
    end

    it 'converts an error to a failure' do
      result = subject.to_failure(error)

      expect(result).to be_a(Temporalio::Api::Failure::V1::Failure)
  ...
  describe '#from_payloads' do
    ...
    it 'returns original payload values' do
      payloads = Temporalio::Api::Common::V1::Payloads.new(payloads: [json_payload, nil_payload])
    ...
    context 'with payload codecs' do
      ...
      it 'decodes the payloads' do
        mixed_payloads = test_codec.encode([json_payload, nil_payload])
        payloads = Temporalio::Api::Common::V1::Payloads.new(payloads: mixed_payloads)
  ...
  describe '#from_failure' do
    ...
    it 'converts a failure to an error' do
      ...
      expect(result).to be_a(Temporalio::Error::ApplicationError)
